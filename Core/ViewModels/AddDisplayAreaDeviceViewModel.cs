...
{
    public partial class AddDisplayAreaDeviceViewModel : ObservableObject
    {
        private readonly HomeViewModel _homeViewModel;
        private readonly ILogger<AddDisplayAreaDeviceViewModel> _logger;

        [ObservableProperty]
        private string deviceName = string.Empty;

        [ObservableProperty]
        private string esp32MqttId = "ESP32_DISPLAY_RS485";

        [ObservableProperty]
        private int slaveId = 50;

        [ObservableProperty]
        private string statusMessage = string.Empty;

        public AddDisplayAreaDeviceViewModel(HomeViewModel homeViewModel, ILogger<AddDisplayAreaDeviceViewModel> logger)
        {
        ...
        private async Task AddDeviceAsync()
        {
            ...
            if (SlaveId < 1 || SlaveId > 247)
            {
                ...
                MessageBox.Show("Slave ID 必須在 1-247 之間\n\n建議展示區使用 1-50 範圍", "驗證錯誤", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                bool success = await _homeViewModel.AddDisplayAreaDeviceAsync(DeviceName, Esp32MqttId, (byte)SlaveId);

                if (success)
                {
                    StatusMessage = $"成功添加設備：{DeviceName}";
                    _logger.LogInformation("成功通過對話框添加展示區設備：{DeviceName} (ESP32: {Esp32Id}, Slave: {SlaveId})",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "通過對話框添加展示區設備時發生錯誤");
